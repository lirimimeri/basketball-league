...
class BasketballLeague extends Server {
  ...
  _onMessage(uid, payload) {
    ...
    switch (cmd) {
      case "ping":
        this._pingFallback(uid);
        break;
      case "topic:sub":
        this._subscribeTopic(uid, data);
        break;
      case "topic:unsub":
        this._unsubscribeTopic(uid, data);
        break;
      case "topic:write":
        this._writeTopic(uid, data.topic, data.payload);
        break;
      case "topic:create":
        this._createTopic(uid, data);
        break;
      default:
        throw new Error("ERR_INVALID_CMD");
    }
  }
  ...
  _closeClient(uid) {
    ...
    if (
      this._sockets[uid] &&
      this._sockets[uid].readyState === WebSocketNode.OPEN
    ) {
      this._sockets[uid].close();
      delete this._sockets[uid];
      Object.keys(this._channels).forEach((key) => {
        this._channels[key].uids = this._channels[key].uids.filter(
          (x) => x !== uid
        );
      });
    }
...
module.exports = BasketballLeague;
